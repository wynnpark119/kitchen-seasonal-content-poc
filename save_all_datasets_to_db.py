#!/usr/bin/env python3
"""
수집 완료된 모든 Apify dataset을 데이터베이스에 저장
...
logger = setup_logger("save_all_datasets")

# 확인된 수집 완료 dataset (키워드별)
...
def save_dataset(dataset_id: str, keyword: str, run_id: int):
    ...
    # 그 다음 process_apify_results(items, keyword, run_id) 호출
...
def main():
    ...
    for dataset_info in COLLECTED_DATASETS:
        ...
        logger.info("→ MCP 도구를 사용하여 데이터 가져오기 필요")
        save_dataset(dataset_id, keyword, run_id)
